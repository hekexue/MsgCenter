	...
	socket = null,
		msgCenter = {
			...
			reg: function(data) {
				if (data) {
					if (!data.id) {
						throw new Error("id是必须的");
					}
					if (!data.handlers) {
						throw new Error("handler参数是必须的");
					}
						...
						callback = function(io) {
							...
							skt.socket.on('connect', function(data) {
								...
								skt.emit("clientRegRequest", regInfo, function(res) {
									...
									//执行各个客户端插件的afterLogin事件
									me.execPlugins("afterConnected", [skt, me]);
								});
							});
							skt.on('disconnect', function() {
								var args = Array.prototype.slice.call(arguments, 0);
								args.push(me);
								me.execPlugins("onDisconnect", args);
							});
							skt.on('reconnect', function() {
								var args = Array.prototype.slice.call(arguments, 0);
								args.push(me);
								me.execPlugins("onReconnect", args);
							});
							skt.on('reconnecting', function(nextRetry) {
								var args = Array.prototype.slice.call(arguments, 0);
								args.push(me);
								me.execPlugins("onReconnecting", args);
							});
							skt.on('reconnect_failed', function() {
								var args = Array.prototype.slice.call(arguments, 0);
								args.push(me);
								me.execPlugins("onReconnectFailed", args);
							});
							socket = skt;
						}
					initSocket(callback);
